...
This branch also includes the first step toward a web product:

- `apps/api`: a lightweight Python FastAPI backend
- `apps/web`: a placeholder folder for the future web frontend

The API currently reads directly from the local `db.sqlite` generated by the Rust sync pipeline and exposes starter endpoints for a dashboard UI.

### Python API Quick Start

Create or activate a Python environment, then install the API dependencies:
...
- `GET /api/v1/heart-rate?limit=100`

If your SQLite database is somewhere else, set:

```sh
OPENWHOOP_SQLITE_PATH=/absolute/path/to/db.sqlite
```

Or, if you prefer running from inside the API folder:
